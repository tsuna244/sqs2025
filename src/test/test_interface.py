...
def test_pokemonobj():
    ...
    assert PokemonObj.from_pokemon_name("nonexisting") == {"details": "Pokemon with this Name not found"}
    assert PokemonObj.from_pokemon_name("bulbasaur") == test_result
    ...
    # check for other rarities !! (and points / multiplier)
    assert PokemonObj(493).get_points() == 600 # 120 * 5
    assert PokemonObj(144).get_points() == 180 # 90 * 2
...
def test_generationobj():
    ...
    assert test_obj.__eq__(result_obj)

    with pytest.raises(IndexError) as e_info:
        test_obj.get_pokemon_by_index(-1)
    
    test_poke_obj = test_obj.get_pokemon_by_index(0) 
    assert test_poke_obj.get_id() == 152
    assert test_poke_obj.get_generation() == "generation-ii"

    assert isinstance(test_obj.get_random_pokemon(), PokemonObj)

    # should give error since pokemonlist is now empty
    test_obj._pokemon_list = []
    with pytest.raises(IndexError) as e_info:
        test_obj.get_pokemon_by_index(0)
    
    with pytest.raises(IndexError) as e_info:
        test_obj.get_random_pokemon()
